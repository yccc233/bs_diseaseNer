    ...
    for c_idx in range(len(tags)):
        ...
        if tag[0] == 'B':
            ...
            if tag[2:] != tags[c_idx-1][2:] or tags[c_idx-1][2:] == 'O':
                ...
                if tag_next[2:] != entity_tag:
                    if entity_tag not in item:
                        item[entity_tag] = [entity_name]
                    else:
                        item[entity_tag].append(entity_name)
                    entity_name = ''
    res = {}
    res['code'] = 0
    res['msg'] = "ner parse success"
    res['data'] = item
    return res
...
def convert_single_example(char_line, tag_to_id, max_seq_length, tokenizer, label_line):
    ...
    label_ids = []
    ntokens.append("[CLS]")
    segment_ids.append(0)  # append("O") 或者 append("[CLS]")暂未定
    ...
    ntokens.append("[SEP]")
    segment_ids.append(0)  # append("O") 或者 append("[SEP]")暂未定
